    ...
    class Procedure
    {
		...
		*/
        function __construct()
        {
            ...
            $this->table_name = 'procedure';
            $this->plan_table_name = 'procedure_plan';
            $this->patient_table_link = 'patient_procedure';
        }
        /*
		lista
		Listand pessoas
		...
		*/
        ...
        public function getByDoctor($cpf)
        {
            global $app;
            ...
            WHERE pro.id = pp.procedure_id AND pp.doctor_id = $cpf AND pp.patient_cpf = pac.cpf AND doc.id = pp.doctor_id");
            // $sth = $this->PDO->prepare("SELECT * FROM $this->table_name WHERE cpf = :cpf");
            // $sth->bindValue(':id', $id);
            $sth->execute();
            $result = $sth->fetchAll(\PDO::FETCH_ASSOC);
        ...
        public function getAllSchedule()
        {
            ...
            $sth = $this->PDO->prepare("SELECT pro.*, pp.do_procedure_date, pac.name as patient, doc.color, doc.name as doctor, pp.id as item 
            FROM `procedure` as pro, `patient_procedure` as pp, patient as pac, `doctor` as doc 
            WHERE pro.id = pp.procedure_id AND pp.patient_cpf = pac.cpf AND doc.id = pp.doctor_id");
            // $sth = $this->PDO->prepare("SELECT * FROM $this->table_name WHERE cpf = :cpf");
            // $sth->bindValue(':id', $id);
            $sth->execute();
            $result = $sth->fetchAll(\PDO::FETCH_ASSOC);
        ...
        public function getScheduleById($id)
        {
            ...
            $sth = $this->PDO->prepare("SELECT pro.*, pp.do_procedure_date, pac.name as patient, doc.color, doc.name as doctor, pp.id as item 
            FROM `procedure` as pro, `patient_procedure` as pp, patient as pac, `doctor` as doc 
            WHERE pro.id = pp.procedure_id AND pp.patient_cpf = pac.cpf AND doc.id = pp.doctor_id AND pp.id = $id
            LIMIT 0,1");
            // $sth = $this->PDO->prepare("SELECT * FROM $this->table_name WHERE cpf = :cpf");
            // $sth->bindValue(':id', $id);
            $sth->execute();
        ...
        public function getByNameOrSpeciality($name)
        {
            global $app;
            $sth = $this->PDO->prepare("SELECT * FROM `$this->table_name`
            WHERE
                name LIKE '%$name%' OR specialty LIKE '%$name%'");
            // $sth = $this->PDO->prepare("SELECT * FROM $this->table_name WHERE cpf = :cpf");
            // $sth->bindValue(':id', $id);
            $sth->execute();
            $result = $sth->fetchAll(\PDO::FETCH_ASSOC);
        ...
        {
            global $app;
            $sth = $this->PDO->prepare("SELECT p.name as title, p.* FROM `plan` as p,`procedure_plan` as pp, `procedure` as pro 
            WHERE 
                pro.id = :id AND pp.procedure_id = pro.id AND pp.plan_cnpj = p.cnpj");
            $sth->bindValue(':id', $id);
            $sth->execute();
            $result = $sth->fetchAll(\PDO::FETCH_ASSOC);
            $app->render('default.php', ["data" => $result], 200);
		...
		*/
        ...
        public function linkPlansToProcedure()
        {
            global $app;
            $dados = json_decode($app->request->getBody(), true);
            ...
            foreach ($dados as $dado) {
                ...
                $keys = array_keys($dado); //Paga as chaves do array
                /*
			O uso de prepare e bindValue é importante para se evitar SQL Injection
			*/
            ...
            //Retorna o id inserido
            $app->render('default.php', ["data" => ['status' => $finish]], 200);
        }
        ...
        public function newProcedure()
        {
			...
			O uso de prepare e bindValue é importante para se evitar SQL Injection
			*/
            $sth = $this->PDO->prepare("INSERT INTO $this->patient_table_link (" . implode(',', $keys) . ") VALUES (:" . implode(",:", $keys) . ")");
            foreach ($dados as $key => $value) {
                $sth->bindValue(':' . $key, $value);
            }
            $sth->execute();
		...
		*/
        public function excluir($id)
        {
            ...
            $sth1 = null;
            if ($sth->execute() == 1) {
                $sth1 = $this->PDO->prepare("DELETE FROM `$this->plan_table_name` WHERE procedure_id = :id");
                $sth1->bindValue(':id', $id);
            }

            $app->render('default.php', ["data" => ['status' => $sth1->execute() == 1]], 200);
        }
    }
}
